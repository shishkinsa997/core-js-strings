...
function removeFirstOccurrences(s, v) {
  ...
  return s.slice(0, s.indexOf(v)).concat(s.slice(s.indexOf(v) + v.length));
}
...
function removeLastOccurrences(s, v) {
  if (s.indexOf(v) === -1) return s;
  return s
    .slice(0, s.lastIndexOf(v))
    .concat(s.slice(s.lastIndexOf(v) + v.length));
}
...
function formatTime(m, s) {
  if (m > 60 || s > 60 || m < 0 || s < 0) return '00:00';
...
function orderAlphabetically(s) {
  return [...s].toSorted((a, b) => a.charCodeAt() - b.charCodeAt()).join('');
}
...
function countVowels(s) {
  const matches = s.match(/[aeiouy]/gi);
...
function isPalindrome(s) {
  const a = s.toLowerCase().match(/[A-z]/gi);
  return (
    a
      .slice(0, Math.ceil(a.length / 2))
      .reverse()
      .join('') === a.slice(Math.floor(a.length / 2)).join('')
  );
}
...
function findLongestWord(s) {
  return s.split(' ').toSorted((a, b) => b.length - a.length)[0];
}
...
function invertCase(str) {
  return [...str]
    .map((l) => {
      if (/[A-Z]/.test(l)) return l.toLowerCase();
      if (/[a-z]/.test(l)) return l.toUpperCase();
      return l;
    })
    .join('');
}
...
function extractNameFromTemplate(v) {
  return v.match(/\b(?!Hello, )([A-Za-z]+)\b/gi).join(' ');
}
